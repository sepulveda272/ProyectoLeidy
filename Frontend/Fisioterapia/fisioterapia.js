import { obtenerFisioterapia, nuevoFisioterapia, deleteFisioterapia, editarFisioterapia } from "./API.js";
...
async function mostrarLista(searchTerm = "") {
  const datos = await obtenerFisioterapia();
...
formulario.addEventListener("submit", validarFisioterapia);

function validarFisioterapia(e) {
  e.preventDefault();
  ...
  if (validate(fisioterapia)) {
    alert("Todos los campos son obligatorios");
    return;
  }

  nuevoFisioterapia(fisioterapia).then(() => mostrarLista()); // Actualizar la lista después de agregar
}
...
function borrar(e) {
  if (e.target.closest(".eliminar")) {
    ...
    const idFisioterapia = button.getAttribute("data-id");
    const confir = confirm("¿Desea eliminar esta fisioterapia?");
    if (confir) {
      deleteFisioterapia(idFisioterapia).then(() => mostrarLista()); // Actualizar la lista después de eliminar
    }
  }
}
...
function actualizar(e) {
  if (e.target.closest(".update")) {
    ...
    const datosExistentes = obtenerFisioterapia().then(datos => {
      const datosFisioterapia = datos.find(item => item._id === idActualizar);
      document.querySelector("#Nombre_Laboratorio_Edit").value = datosFisioterapia.Nombre_Laboratorio;
      document.querySelector("#Responsable_Edit").value = datosFisioterapia.Responsable;
      document.querySelector("#Inventario_Edit").value = datosFisioterapia.Inventario.join(", ");
      document.querySelector("#Nombre_Edit").value = datosFisioterapia.Nombre;
      document.querySelector("#Marca_Edit").value = datosFisioterapia.Marca;
      document.querySelector("#Funcion_Edit").value = datosFisioterapia.Funcion;
      document.querySelector("#Procedencia_Edit").value = datosFisioterapia.Procedencia;
    });

    const formEditFisioterapia = document.querySelector("#formEditFisioterapia");
    formEditFisioterapia.addEventListener("submit", (event) => {
      ...
      editarFisioterapia(datos).then(() => mostrarLista());
    }, { once: true });
  }
}
