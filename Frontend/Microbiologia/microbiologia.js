import { obtenerMicrobiologia, nuevoMicrobiologia, deleteMicrobiologia, editarMicrobiologia } from "./API.js";
...
function exportTableToExcel() {
  ...
  XLSX.writeFile(workbook, "tabla_microbiologia.xlsx");
}


/* LISTAR CATEGORIAS - CRUD (R) */
async function mostrarLista(searchTerm = "") {
  const datos = await obtenerMicrobiologia();
...
formulario.addEventListener("submit", validarMicrobiologia);

function validarMicrobiologia(e) {
  e.preventDefault();
  ...
  if (validate(microbiologia)) {
    alert("Todos los campos son obligatorios");
    return;
  }

  nuevoMicrobiologia(microbiologia).then(() => mostrarLista()); // Actualizar la lista después de agregar
}
...
function borrar(e) {
  if (e.target.closest(".eliminar")) {
    ...
    const confir = confirm("¿Desea eliminar esta Microbiologia?");
    if (confir) {
      deleteMicrobiologia(idEnfermeria).then(() => mostrarLista()); // Actualizar la lista después de eliminar
    }
  }
}
...
function actualizar(e) {
  if (e.target.closest(".update")) {
    ...
    const datosExistentes = obtenerMicrobiologia().then(datos => {
      const datosMicrobiologia = datos.find(item => item._id === idActualizar);
      document.querySelector("#Nombre_Laboratorio_Edit").value = datosMicrobiologia.Nombre_Laboratorio;
      document.querySelector("#Responsable_Edit").value = datosMicrobiologia.Responsable;
      document.querySelector("#Inventario_Edit").value = datosMicrobiologia.Inventario.join(", ");
      document.querySelector("#Nombre_Edit").value = datosMicrobiologia.Nombre;
      document.querySelector("#Marca_Edit").value = datosMicrobiologia.Marca;
      document.querySelector("#Funcion_Edit").value = datosMicrobiologia.Funcion;
      document.querySelector("#Procedencia_Edit").value = datosMicrobiologia.Procedencia;
    });

    const formEditMicrobiologia = document.querySelector("#formEditMicrobiologia");
    formEditMicrobiologia.addEventListener("submit", (event) => {
      ...
      editarMicrobiologia(datos).then(() => mostrarLista());
    }, { once: true });
  }
}
